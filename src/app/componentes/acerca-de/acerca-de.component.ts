...
export class AcercaDeComponent implements OnInit {
  ...
  constructor(
    private acercaS: AcercadeService,
  ...
  ngOnInit(): void {
    ...
    this.acercaS.detail(1).subscribe((data) => {
      this.updateacerca = data;
    });
  }

  cargarDatosAcerca(): void {
    this.acercaS.getdato().subscribe((data) => {
      this.acerca = data;
    });
  }
  ...
  actualizarAcerca(): void {
    const id = 1;
    this.acercaS.update(id, this.updateacerca).subscribe(() => {
      this.cargarDatosAcerca();
    });
  }
}
